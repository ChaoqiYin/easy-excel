...
class ExportWorkBook(object):
    def __init__(self, file_path_or_stream, converters, sheet_map, before=None, after=None, style=None,
    ...
    def __start_parse(self, sheet_map, before, after):
        # 升序排序
        for index, sm in sheet_map.items():
            rel_row_del_class = sm.row_del_class or ExportRow
            ExportSheet(self, self.workbook, index, sm, max_workers=self.max_workers, row_del_class=rel_row_del_class).parse_export(before, after)
